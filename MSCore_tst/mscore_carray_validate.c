...
static inline int carray_create(void)
  ...
  for (i= 0; i<10; i++) {
    ...
    RELEASE(b);}
  err+= ASSERT_EQUALS(RETAINCOUNT(x), 56, "Bad retain count(1): %lu");
  RELEASE(a);
  err+= ASSERT_EQUALS(RETAINCOUNT(x), 1, "Bad retain count(2): %lu");
  RELEASE(x);
  return err;
  }
...
static inline int carray_ptr(void)
  ...
  err+= ASSERT(i == 77 && p[i] == 154, "Bad index(3): %lu");
  ...
  err+= ASSERT(i==13 && p[i]==26 && q[i]==p+i, "Bad index(4): %lu");
  // insert
  // p: 0 2 .. 198 1 3 .. 199
  // a: 0 1 .. .. .. 198 199
...
//fprintf(stdout, "\n");
  ...
  err+= ASSERT(i==195 && *((NSInteger*)a->pointers[i])==195 && q[97]==p+197, "Bad index(5): %lu");
  // remove
  CArrayRemoveObjectsInRange(a, NSMakeRange(10, 20));
  err+= ASSERT_EQUALS(CArrayCount(a), 180, "Bad count(6): %lu");
  x= (NSUInteger*)CArrayObjectAtIndex(a, 10);
  err+= ASSERT_EQUALS(*x, 30, "Bad int (7): %lu");
  i= CArrayIndexOfIdenticalObject(a, (id)x, 0, CArrayCount(a));
  err+= ASSERT_EQUALS(i, 10, "Bad index(8): %lu");
  // replace
  ...
  err+= ASSERT_EQUALS(i, 2, "Bad remove identical(9): %lu");
  i= CArrayIndexOfIdenticalObject(a, (id)q[4], 0, CArrayCount(a)); // 9
  err+= ASSERT_EQUALS(i, 8, "Bad index(10): %lu");
  i= CArrayIndexOfIdenticalObject(a, (id)q[50], 0, CArrayCount(a)); // 101
  err+= ASSERT_EQUALS(i, 101-20-2, "Bad index(11): %lu");
  RELEASE(a);
  return err;
  }
...
static inline int carray_subarray(void)
  ...
  err+= ASSERT_EQUALS(((CArray*)o)->count, 1, "Bad count(30): %lu");
  for (i=0; i<n; i++) {
    b= CCreateArray(i+1);
    ...
    RELEASE(b);}
  RELEASE(o);
  err+= ASSERT_EQUALS(RETAINCOUNT(o), n*(n+1)/2, "Bad retain count(31): %lu");
  b= CCreateSubArrayWithRange(a, NSMakeRange(n/2, n/3));
  err+= ASSERT_EQUALS(b->count, n/3, "Bad count(32): %lu");
  err+= ASSERT_EQUALS(RETAINCOUNT(o), n*(n+1)/2, "Bad retain count(33): %lu");
  c= CCreateArray(n/3);
  CArrayAddArray(c, b, YES);
  err+= ASSERT_EQUALS(RETAINCOUNT(o), n*(n+1)/2+(4*n+3)*n/18, "Bad retain count(34): %lu");
  RETAIN(o);
  RELEASE(a);
  RELEASE(b);
  RELEASE(c);
  err+= ASSERT_EQUALS(RETAINCOUNT(o), 1, "Bad retain count(35): %lu");
  RELEASE(o);
  return err;
  }
