...
static void ccouple_create(test_t *test)
  ...
  CCoupleSetFirstMember(e, d);
  TASSERT(test, !CCoupleEquals(f, e), "f & e are not equals anymore");
  TASSERT_EQUALS_PTR(test, CCoupleFirstMember(e), d);
  TASSERT_EQUALS_PTR(test, CCoupleSecondMember(e), d);
  TASSERT_EQUALS_LLD(test, RETAINCOUNT(c), 3);
  TASSERT_EQUALS_LLD(test, RETAINCOUNT(d), 4);

  CCoupleSetSecondMember(e, c);
  TASSERT(test, !CCoupleEquals(f, e), "f & e are not equals anymore");
  TASSERT_EQUALS_PTR(test, CCoupleFirstMember(e), d);
  TASSERT_EQUALS_PTR(test, CCoupleSecondMember(e), c);
  TASSERT_EQUALS_LLD(test, RETAINCOUNT(c), 4);
  TASSERT_EQUALS_LLD(test, RETAINCOUNT(d), 3);

  CCoupleSetFirstMember(f, d);
  CCoupleSetSecondMember(f, c);
...
static void ccouple_toarray(test_t *test)
{
  ...
  a0= CCreateArrayOfCoupleSubs(c0, nil);
  a1= CCreateArrayOfCoupleSubs(c1, nil);
  a2= CCreateArrayOfCoupleSubs(c2, nil);

  TASSERT_EQUALS_LLD(test, CArrayCount(a0), 0);

  TASSERT_EQUALS_LLD(test, CArrayCount(a1), 1);
  TASSERT_EQUALS_PTR(test, CArrayObjectAtIndex(a1, 0), c0);

  TASSERT_EQUALS_LLD(test, CArrayCount(a2), 2);
  TASSERT_EQUALS_PTR(test, CArrayObjectAtIndex(a2, 0), c0);
  TASSERT_EQUALS_PTR(test, CArrayObjectAtIndex(a2, 1), c1);
