 ...
 *   1) all functions have now a context parameter
 *   2) added const to necessary function parameters
 *   3) replaces local stacked vars with local initialized vars
 */
...
/****************************************************************************/
void m_apm_divide(M_APM rr, int places, const M_APM aa, const M_APM bb)
{
  M_APM   tmp0, tmp1;
  int     sn, nexp, dplaces;
  ...
  {
    M_set_to_zero(rr);
    if (bb->m_apm_sign == 0) { M_apm_log_error_msg(M_APM_DIVIDE_BY_ZERO, "\'m_apm_divide\', Divide by 0"); }
    return;
  }
...
/****************************************************************************/
void m_apm_reciprocal(M_APM rr, int places, const M_APM aa)
{
  ...
  if (aa->m_apm_sign == 0) {
    M_set_to_zero(rr);
    M_apm_log_error_msg(M_APM_DIVIDE_BY_ZERO, "\'m_apm_reciprocal\', Input = 0");
    return;
  }
