...
#define M_class_copy_error_msg   "\'CDecimalCopy\', Out of memory"
void MSExport CDecimalFree(void *self)
{
  if (self && m_apm_deallocate((M_APM)self)) {
    MSFree(self, "freeing CDecimal object") ;
  }
}

BOOL CDecimalIsEqual(void *self, void *other)
{
  if (self == other) { return YES ; } ;
  if (self && other && ((CDecimal *)self)->isa == ((CDecimal *)other)->isa) {
    return m_apm_compare((const M_APM)self, (const M_APM)other) == 0 ? YES : NO ;
  }
  return NO ;
}

id CDecimalCopy(void *self)
{
  CDecimal *newObject = (CDecimal *)MSCreateObjectWithClassIndex(CDecimalClassIndex) ;
  if (newObject) {
    newObject->m_apm_id           = M_APM_IDENT;
    newObject->m_apm_malloclength = 80;
    newObject->m_apm_datalength   = 1;
    newObject->m_apm_exponent     = 0;
    newObject->m_apm_sign         = 0;
    if ((newObject->m_apm_data = (UCHAR *)MAPM_MALLOC(84)) == NULL) {
      MAPM_FREE(newObject) ; newObject = NULL ;
      M_apm_log_error_msg(M_APM_INIT_ERROR, M_class_copy_error_msg);
    }
    else {
      newObject->m_apm_data[0] = 0;
    }
    m_apm_copy((M_APM)newObject, (const M_APM)self) ;
  }
  return newObject ;
}

NSUInteger CDecimalHash(void *self, unsigned depth)
{
  depth= 0;
  // TODO:
  ...
  return (NSUInteger)self ; // THIS VALUE IS BAD : MUST BE DONE CORRECTLY BEFORE TO BE USED IN CDICTIONARIES
}
