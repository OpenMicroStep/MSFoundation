 ...
 *   1) removed M_free_all_div() since the they are no more statics here
 *   2) added const to necessary function parameters
 *   3) removed temporary variables M_div_worka, M_div_workb, M_div_tmp7, M_div_tmp8, M_div_tmp9 (replaced by locals)
 */
...
/****************************************************************************/
void m_apm_integer_divide(M_APM rr, const M_APM aa, const M_APM bb)
{
  /*
...
void M_apm_sdivide(M_APM r, int places, const M_APM a, const M_APM b)
{
  ...
  {
    M_set_to_zero(r);
    if (b->m_apm_sign == 0) { M_apm_log_error_msg(M_APM_DIVIDE_BY_ZERO, "\'M_apm_sdivide\', Divide by 0"); }
    return;
  }
