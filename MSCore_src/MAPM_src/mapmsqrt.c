...
/****************************************************************************/
void m_apm_sqrt(M_APM rr, int places, const M_APM aa)
{
  ...
  if (aa->m_apm_sign <= 0) {
    if (aa->m_apm_sign == -1) {
      M_apm_log_error_msg(M_APM_BAD_SQRT, "\'m_apm_sqrt\', Negative argument");
    }
  ...
  while (TRUE) {
    ...
    if (ii != 0) {
      ...
      if ((-4 * tmp7->m_apm_exponent) > tolerance) { bflag = TRUE; }
    }
  ...
  m_apm_free(tmp7); m_apm_free(tmp8); m_apm_free(tmp9); m_apm_free(tmpN);
  m_apm_free(guess); m_apm_free(last_x);
}
/****************************************************************************/
