...
void M_raw_sin(M_APM rr, int places, const M_APM xx)
{
  ...
  while (TRUE)
  {
    ...
    if (local_precision < 20) local_precision = 20;
  ...
  m_apm_free(sum); m_apm_free(term);
  m_apm_free(tmp2); m_apm_free(tmp7); m_apm_free(tmp8);
}
/****************************************************************************/
/*
 x^2     x^4     x^6     x^8
 ...
 2!      4!      6!      8!
 */
void M_raw_cos(M_APM rr, int places, const M_APM xx)
{
