 ...
 *   1) all statics used for calculation are transformed in context defines and inits are in context init
 *   2) all necessary functions have now a M_APM_FMC context parameter
 *   3) M_free_all_fmul() is removed since the context manages the deallocation of its variables
 *   4) added const to necessary function parameters
 *   5) added functions M_init_fmul_context() and M_free_fmul_context() to manage the fmul context
 */
...
#define exp_stack           (context->_exp_stack)
#define exp_stack_ptr       (context->_exp_stack_ptr)

#define mul_stack_data      (context->_mul_stack_data)
#define mul_stack_data_size (context->_mul_stack_data_size)
#define M_mul_stack_ptr     (context->_M_mul_stack_ptr)

#define M_stack_ptr_error_msg "\'M_get_stack_ptr\', Out of memory"

extern void M_fast_multiply(M_APM, M_APM, M_APM);
extern void M_fmul_div_conq(UCHAR *, UCHAR *, UCHAR *, int, M_APM_FMC);
...
extern int  M_get_stack_ptr(int, M_APM_FMC);
extern void M_push_mul_int(int, M_APM_FMC);
extern int  M_pop_mul_int(M_APM_FMC);
...
/****************************************************************************/
int M_pop_mul_int(M_APM_FMC context)
{
...
/****************************************************************************/
void M_fast_multiply(M_APM rr, const M_APM aa, const M_APM bb)
{
  void *vp;
  int ii, k, nexp, sign;
  M_APM_FMC context ;
...
#ifdef NO_FFT_MULTIPLY
  M_fmul_div_conq(rr->m_apm_data, M_ain->m_apm_data, M_bin->m_apm_data, ii, context);
#else
  ...
  if (MM_SIZEOF_INT == 2) {   /* if still using 16 bit compilers .... */
    M_fast_mul_fft(rr->m_apm_data, M_ain->m_apm_data, M_bin->m_apm_data, ii);
  }
 ...
 *      data that needs to be saved is saved on one of the two stacks.
 ...
 HM: OK this design is a nightmare in multithread environment.
 First we put any local variable we can here
 */
void M_fmul_div_conq(UCHAR *rr, UCHAR *aa, UCHAR *bb, int sz, M_APM_FMC context)
{
  ...
  itmp = M_get_stack_ptr(mii, context);
  M_push_mul_int(itmp, context);
  ...
  itmp    = M_get_stack_ptr(2 * sz, context);
  ...
  M_push_mul_int(stmp, context);
  M_push_mul_int(mii, context);
  
  M_fmul_div_conq(fmul_t0, fmul_a0, fmul_b0, mii, context);
  
  mii  = M_pop_mul_int(context);
  stmp = M_pop_mul_int(context);
  itmp = M_pop_mul_int(context);
  ...
  M_fmul_div_conq(fmul_t0, fmul_a1, fmul_b1, mii, context);
  ...
  if (stmp != 0)
    M_fmul_div_conq(fmul_t0, fmul_a9, fmul_b9, mii, context);
...
/****************************************************************************/
int M_get_stack_ptr(int sz, M_APM_FMC context)
{
  int i, k;
  UCHAR   *cp;
  ...
  if (mul_stack_data_size[k] == 0)
  {
    ...
    if ((cp = (UCHAR *)MAPM_MALLOC((size_t)(i + 4))) == NULL)
    {
      /* fatal, this does not return */
      
      M_apm_log_error_msg(M_APM_MALLOC_ERROR, M_stack_ptr_error_msg);
    }
  ...
  else        /* it has been malloc'ed, see if it's big enough */
  {
    if (sz > mul_stack_data_size[k])
    {
      ...
      if ((cp = (UCHAR *)MAPM_REALLOC(cp, (size_t)(sz + 4))) == NULL)
      {
        /* fatal, this does not return */
...
void M_4_byte_multiply(UCHAR *r, UCHAR *a, UCHAR *b)
{
  ...
  memset(rr, 0, (8 * MM_SIZEOF_INT));        /* zero out result */
  jj = 3;
  ip = rr + 5;
