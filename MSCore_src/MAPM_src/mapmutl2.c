 ...
 *   The modifications are :
     ...
     2) converted M_long_2_ascii() in M_ulong_2_ascii() which takes an unsigned long long as an argument and a sign
 */
...
/****************************************************************************/
int m_apm_sign(const M_APM atmp)
{
  return(atmp->m_apm_sign);
}

/****************************************************************************/
int m_apm_exponent(const M_APM atmp)
{
...
/****************************************************************************/
int m_apm_significant_digits(const M_APM atmp)
{
  return(atmp->m_apm_datalength);
}
/****************************************************************************/
int m_apm_is_integer(const M_APM atmp)
{
  if (atmp->m_apm_sign == 0)
    return(1);
...
/****************************************************************************/
int  m_apm_is_even(const M_APM aa)
{
  int     ii, jj;
...
/****************************************************************************/
int  m_apm_is_odd(const M_APM bb)
{
  if (m_apm_is_even(bb))
    return(0);
  else
...
/****************************************************************************/
void m_apm_negate(M_APM d, const M_APM s)
{
  m_apm_copy(d,s);
  if (d->m_apm_sign != 0)
...
/****************************************************************************/
void m_apm_absolute_value(M_APM d, const M_APM s)
{
  m_apm_copy(d,s);
  if (d->m_apm_sign != 0)
    d->m_apm_sign = 1;
...
/****************************************************************************/
void m_apm_copy(M_APM dest, const M_APM src)
{
  int j;
  void *vp;
  ...
  {
    if ((vp = MAPM_REALLOC(dest->m_apm_data, (size_t)(j + 32))) == NULL)
    {
      /* fatal, this does not return */
      
      M_apm_log_error_msg(M_APM_MALLOC_ERROR, "\'m_apm_copy\', Out of memory");
    }
...
/****************************************************************************/
int m_apm_compare(const M_APM ltmp, const M_APM rtmp)
{
 ...
 *
 */
// TODO: M_ulong_2_ascii() to be rewrited
void    M_ulong_2_ascii(char *output, unsigned long long input, int sign)
{
  ...
  if ((m > t) || (m < -t)) {  /* handle the bigger numbers with 'sprintf'. */
    ...
    sprintf(p, "%llu", m);  /* at 'LONG_MIN', etc.                       */
  }
  else {
    
    if (sign < 0) {             /* handle the sign */
 ...
 */
int     M_strposition(const char *s1, const char *s2)
{
