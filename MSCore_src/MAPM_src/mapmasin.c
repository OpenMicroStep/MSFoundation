...
/****************************************************************************/
void m_apm_arctan2(M_APM rr, int places, const M_APM yy, const M_APM xx)
{
  ...
  if (ix == 0)       /* x == 0 */
  {
    if (iy == 0)    /* y == 0 */
    {
      M_apm_log_error_msg(M_APM_BAD_TRIGO, "\'m_apm_arctan2\', Both Inputs = 0");
      M_set_to_zero(rr);
      return;
    }
  ...
  m_apm_free(tmp5); m_apm_free(tmp6); m_apm_free(tmp7);
}
/****************************************************************************/
/*
 ...
 */
void m_apm_arctan(M_APM rr, int places, const M_APM xx)
{
  M_APM   tmp8, tmp9;
 ...
 */
void M_arctan_large_input(M_APM rr, int places, const M_APM xx)
{
  M_APM tmp1, tmp2;
...
void m_apm_arcsin(M_APM r, int places, const M_APM x)
{
  ...
  while (TRUE) {
    ...
    if (tmp2->m_apm_sign != 0) tmp2->m_apm_sign = current_x->m_apm_sign;
    ...
    if (ii != 0) {
      if (((2 * tmp0->m_apm_exponent) < tolerance) || (tmp0->m_apm_sign == 0)) break;
    }
...
void m_apm_arccos(M_APM r, int places, const M_APM x)
{
  ...
  if (ii == 0) {       /* |x| == 1, arccos = 0, PI */
    if (x->m_apm_sign == 1) { M_set_to_zero(r); }
    else {
      M_check_PI_places(places);
