...
Date.prototype.dateWithoutTime = function() {
	var utc = this.getUTCSeconds() ;
    return Date.initWithUTCSeconds(utc - (utc % 86400)) ;
} ;
...
MSData.prototype.bitAtIndex = function(i) {
	if (i >= 0) {
		...
		if (charIndex < this.length) {
			return (MSData.base64Tokens[this.base64String.charAt(charIndex)] & (1 << (5 - (i % 6)))) ? true : false ;
		}
	}
	return false ;
} ;
